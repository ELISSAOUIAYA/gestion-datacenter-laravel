...
<div style="background: #f5f7fa; min-height: 100vh; padding: 2rem;">
    <div style="max-width: 700px; margin: 0 auto;">
        ...
        <div style="background: white; padding: 2rem; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
            <h1 style="margin: 0 0 2rem 0; color: #1a1a1a;">Éditer Maintenance</h1>
            ...
            <form method="POST" action="{{ route('admin.maintenances.update', $maintenance) }}">
                ...
                <div style="margin-bottom: 1.5rem;">
                    ...
                    <select name="resource_id" style="width: 100%; padding: 0.75rem; border: 1px solid #ddd; border-radius: 6px;" required>
                        ...
                        <option value="{{ $res->id }}" {{ $maintenance->resource_id === $res->id ? 'selected' : '' }}>{{ $res->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1.5rem;">
                    <div>
                        ...
                        <input type="datetime-local" name="start_date" value="{{ $maintenance->start_date->format('Y-m-d\TH:i') }}" style="width: 100%; padding: 0.75rem; border: 1px solid #ddd; border-radius: 6px;" required>
                    </div>

                    <div>
                        <label style="display: block; font-weight: 600; margin-bottom: 0.5rem; color: #333;">Fin</label>
                        <input type="datetime-local" name="end_date" value="{{ $maintenance->end_date->format('Y-m-d\TH:i') }}" style="width: 100%; padding: 0.75rem; border: 1px solid #ddd; border-radius: 6px;" required>
                    </div>
                </div>

                <div style="margin-bottom: 1.5rem;">
                    <label style="display: block; font-weight: 600; margin-bottom: 0.5rem; color: #333;">Description</label>
                    <textarea name="description" style="width: 100%; padding: 0.75rem; border: 1px solid #ddd; border-radius: 6px; min-height: 100px;">{{ $maintenance->description }}</textarea>
                </div>
